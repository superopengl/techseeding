...
- `PATCH /api/tutor/:sessionId` — update a session in place. Body accepts any combination of `subject` and `currentDocId` (set to a doc UUID owned by this session, or `null` to clear). Validates each field against its enum, verifies the target doc belongs to the session, and returns the patched fields. Tutor pacing is no longer a session attribute — it rides on each `POST /api/tutor/:sessionId/message` body as `guidanceLevel`.
- `GET /api/tutor/:sessionId/messages` — fetch transcript. Each `user` row carries the `guidanceLevel` the student picked for that turn (legacy rows have it null).
- `POST /api/tutor/:sessionId/message` — send chat message; streams Brain's response over SSE. Each page of the active doc is attached to the user message as an `image_url` block so the multimodal Brain reads the worksheet directly. Brain may call `draw_annotation` to point at the page; per-turn freehand-canvas bytes override the original photo when the student drew on it for that turn. The request body's `guidanceLevel` (`guided` | `balanced` | `direct`) sets Brain's pacing for this single turn and is persisted on the inserted user row; missing or invalid values fall back to `direct`.
