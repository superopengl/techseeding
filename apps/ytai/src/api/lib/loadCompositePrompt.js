...
// Compose the three editable tiers into the raw composite string, in the
// canonical global → subject → year order. Shared by the publish route (as
// the source it refines) and the runtime fallback below.
export async function composeRawPrompt(year, subject) {
  const { global, subject: subjectPrompt, year: yearPrompt } = await loadAgentPrompt(
    year,
    subject
  );
  return [global, subjectPrompt, yearPrompt]
    .map((s) => (s || '').trim())
    .filter(Boolean)
    .join('\n\n');
}
