...
if [ -z "$KPAI_DATABASE_URL" ] && [ -n "$KPAI_PG_HOST" ]; then
  ...
  export KPAI_DATABASE_URL="postgresql://${KPAI_PG_USER}:${KPAI_PG_PASSWORD}@${KPAI_PG_HOST}:${KPAI_PG_PORT}/${KPAI_PG_DATABASE}?sslmode=require"
fi
...
if [ "$RUN_MIGRATIONS" = "true" ]; then
  echo "Running database migrations against ${KPAI_PG_HOST:-(KPAI_DATABASE_URL provided)}..."
  attempt=1
  max_attempts=12
  while true; do
