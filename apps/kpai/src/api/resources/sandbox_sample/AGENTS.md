...
- Do NOT modify AGENTS.md, opencode.json, nono-opencode-profile.json, robots.txt.
- Do NOT attempt to access, read, or reference any other file or path.

## Forbidden Actions

- **No file creation.** Do NOT create any new files. All code must go inside `index.html`.
- **No directory listing.** Do NOT list files or directories. Do NOT use `ls`, `find`, `tree`, or any equivalent.
- **No shell commands.** Do NOT run any shell commands. This includes but is not limited to: `pwd`, `whoami`, `date`, `time`, `env`, `echo`, `curl`, `wget`, `node`, `python`, `npm`, `cat`, `touch`, `mkdir`, `rm`, `cp`, `mv`, `chmod`, or any other command.
