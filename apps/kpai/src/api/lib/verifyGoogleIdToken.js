...
// endpoint. Avoids pulling in `google-auth-library` + a JWKS cache; the
// tradeoff is one outbound HTTP per sign-in. Google validates signature,
// expiry, and issuer for us and returns the claims.
//
// Throws on any failure (network, invalid token, audience mismatch). On
// success returns { sub, email, name, picture, aud, iss }.
